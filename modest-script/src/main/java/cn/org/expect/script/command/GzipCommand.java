...
public class GzipCommand extends AbstractFileCommand implements UniversalScriptInputStream, JumpCommandSupported, NohupCommandSupported {
    ...
    public void read(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptParser parser, UniversalScriptAnalysis analysis, Reader in) throws IOException {
        if (analysis.isBlankline(this.filepath)) {
            ...
            throw new UniversalScriptException(ResourcesUtils.getMessage("script.message.stderr014", this.command, "gzip", this.filepath));
        }
    }
    ...
    public int execute(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptStdout stdout, UniversalScriptStderr stderr, boolean forceStdout, File outfile, File errfile) throws Exception {
        ...
        File gzfile = new File(file.getParentFile(), FileUtils.changeFilenameExt(file.getName(), "gz"));
        this.c = context.getContainer().getBean(Compress.class, "gz");
        try {
            this.c.setFile(gzfile);
            this.c.archiveFile(file, null);

            session.removeValue();
            session.putValue("file", gzfile);
