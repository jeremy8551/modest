...
public class RenameMarkdown {
    ...
    public void rename(String markdownFilepath, String newFilename) throws IOException {
        ...
        Logs.info("markdown.stdout.message006", imageDir.getName(), oldDecodeName);
        Ensure.isTrue(markdownFile.renameTo(newMarkdownFile));

        if (!imageDir.exists()) {
            Logs.info(imageDir.getAbsolutePath());
            return;
        }
        ...
        Logs.info("markdown.stdout.message006", newImgDirName, dest);
        ...
        if (Logs.isInfoEnabled()) {
            Logs.info("markdown.stdout.message007", markdownFile.getAbsolutePath(), newMarkdownFile.getName());
            Logs.info("markdown.stdout.message007", imageDir.getAbsolutePath(), newImgDir.getName());
        }
