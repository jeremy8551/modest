    ...
    private void rename(File file) throws IOException, ModestException {
        ...
        if (StringUtils.inArrayIgnoreCase(extName, "md")) {
            ...
            if (imgDir.exists() && imgDir.isDirectory()) {
                ...
                if (number > 0) {
                    Logs.info("markdown.stdout.message002", imgDir.getAbsolutePath(), number);
                }
                ...
                for (File imgfile : files) {
                    ...
                    if (StringUtils.isBlank(FileUtils.getFilenameSuffix(imgfile.getName()))) {
                        Logs.info("markdown.stdout.message003", imgfile.getAbsolutePath());
                    }
                    ...
                    if (content.contains(oldImageName)) {
                        ...
                        Logs.info("markdown.stdout.message005", imgfile.getAbsolutePath(), newImagefile.getAbsolutePath(), (FileUtils.rename(imgfile, newImagefile, null) ? "[success]" : "[fail]"));
                        continue;
                    }
                }
            }
