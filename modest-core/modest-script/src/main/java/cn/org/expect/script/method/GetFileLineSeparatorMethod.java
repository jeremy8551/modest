...
@ScriptFunction(name = "getFileLineSeparator")
public class GetFileLineSeparatorMethod extends AbstractMethod {
    ...
    public int execute(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptStdout stdout, UniversalScriptStderr stderr, UniversalScriptAnalysis analysis, String variableName, String methodHandle) throws Exception {
        if (methodHandle.charAt("getFileLineSeparator".length()) != '(') {
        ...
        int funcStart = "getFileLineSeparator(".length();
        ...
        if (value == null) {
            ...
            this.value = FileUtils.readLineSeparator(file);
            int next = funcEnd + 1;
