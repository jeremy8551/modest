...
@ScriptFunction(name = "getFilenameNoExt")
public class GetFilenameNoExtMethod extends AbstractMethod {
    ...
    public int execute(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptStdout stdout, UniversalScriptStderr stderr, UniversalScriptAnalysis analysis, String variableName, String methodHandle) throws Exception {
        if (methodHandle.charAt("getFilenameNoExt".length()) != '(') {
        ...
        int funcStart = "getFilenameNoExt(".length();
        ...
        if (value == null) {
            ...
            this.value = FileUtils.getFilenameNoExt(str);
            int next = funcEnd + 1;
