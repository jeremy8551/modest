...
@ScriptFunction(name = "getFileSuffix")
public class GetFileSuffixMethod extends AbstractMethod {
    ...
    public int execute(UniversalScriptSession session, UniversalScriptContext context, UniversalScriptStdout stdout, UniversalScriptStderr stderr, UniversalScriptAnalysis analysis, String variableName, String methodHandle) throws Exception {
        if (methodHandle.charAt("getFileSuffix".length()) != '(') {
        ...
        int funcStart = "getFileSuffix(".length();
        ...
        if (value == null) {
            ...
            this.value = FileUtils.getFilenameSuffix(str);
            int next = funcEnd + 1;
