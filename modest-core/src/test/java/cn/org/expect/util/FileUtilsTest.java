...
public class FileUtilsTest {
    ...
    public void testSpellFileStringString() {
        Assertions.assertEquals("/home/test" + File.separator + "shell", FileUtils.joinPath("/home/test", "shell"));
        Assertions.assertEquals("/home/test" + File.separator + "shell/qyzx", FileUtils.joinPath("/home/test", "shell/qyzx"));
    }

    @Test
    public void testSpellFileStringArray() {
        Assertions.assertEquals("home" + File.separator + "test" + File.separator + "shell" + File.separator + "grzx", FileUtils.joinPath(new String[]{"home", "test", "shell", "grzx"}));
    }

    @Test
    public void testJoinPath() {
        Assertions.assertEquals(File.separator + "home" + File.separator + "User" + File.separator + "cn" + File.separator + "org" + File.separator + "expect" + File.separator + "util", FileUtils.joinPath(File.separator + "home" + File.separator + "User", FileUtilsTest.class));
    }
    ...
    void findUpward() {
        ...
        Assertions.assertEquals(dest, upward);
    }
    ...
    public void testEqualsIgnoreLineSeparator() throws IOException {
        ...
        Assertions.assertTrue(FileUtils.write(f0, CharsetUtils.get(), false, "1"));
        Assertions.assertTrue(FileUtils.write(f1, CharsetUtils.get(), false, "1"));
        Assertions.assertEquals(0, FileUtils.equalsIgnoreLineSeparator(f0, CharsetUtils.get(), f1, CharsetUtils.get(), 0));
    ...
    public void test100() throws IOException, InterruptedException {
        ...
        final String charsetName = CharsetName.UTF_8;
        Assertions.assertTrue(FileUtils.write(file, charsetName, false, "testset"));
        Logs.info(FileUtils.readline(file, charsetName, 0));
        ...
        Logs.info("主线程恢复运行 ..");
        Logs.info(" after: " + FileUtils.readline(file, charsetName, 0));
    ...
    public void testCodepages() throws IOException {
        ...
        System.setProperty(Codepage.PROPERTY_CODEPAGE, userDefine.getAbsolutePath());
     ...
     * 测试用的Process实现类
     */
    private static class TestProcess implements FileUtils.Process {
        ...
        public boolean match(String filename, String expression) {
            ...
            if (expression.contains("*")) {
                ...
                // 简单的单字符匹配
                if (expression.length() != filename.length()) {
                    return false;
                }
                for (int i = 0; i < expression.length(); i++) {
                    if (expression.charAt(i) != '?' && expression.charAt(i) != filename.charAt(i)) {
                        return false;
                    }
                }
                return true;
            } else {
                return filename.equals(expression);
            }
        }
    ...
    // TODO
    @Test
    public void testHandlePathExpression() throws Exception {
        // 创建测试目录和文件
        File tempDir = FileUtils.createTempDirectory(null);
        ...
        File file3 = new File(testDir, "data.csv");
        ...
        FileUtils.handlePathExpression(testDir.getAbsolutePath() + "/test1.txt", process1);
        Assertions.assertEquals(1, process1.getExecuteCount());
        Assertions.assertEquals("test1.txt", process1.getLastFileName());

        // 测试通配符匹配 *
        TestProcess process2 = new TestProcess();
        FileUtils.handlePathExpression(testDir.getAbsolutePath() + "/test*.txt", process2);
        Assertions.assertEquals(2, process2.getExecuteCount());

        // 测试通配符匹配 ?
        TestProcess process3 = new TestProcess();
        FileUtils.handlePathExpression(testDir.getAbsolutePath() + "/test?.txt", process3);
        Assertions.assertEquals(2, process3.getExecuteCount());
