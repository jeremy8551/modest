...
public class MavenFinderThread extends Thread {
    ...
    public MavenFinderThread(MavenFinder mavenFinder) {
        super();
        this.setName(MavenFinder.class.getSimpleName());
        this.mavenFinder = Ensure.notNull(mavenFinder);
    }

    @Override
    public void run() {
        log.info(MavenFinderMessage.START_THREAD.fill(this.getName()));
        ...
        log.info(MavenFinderMessage.DETECTED_IDEA_UI_COMPONENT.fill(this.getName()));
        this.mavenFinder.addPopupMenu();
