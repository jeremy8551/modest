...
import cn.org.expect.maven.repository.central.CentralRepositoryDatabase;
import cn.org.expect.maven.repository.central.CentralRepositoryDatabaseSerializer;
import cn.org.expect.maven.repository.central.ExtraResultAnalysis;
import cn.org.expect.maven.repository.central.PatternResultAnalysis;
 ...
 */
@EasyBean("gradle")
public class GradleRepository implements MavenRepository {
    protected final static Log log = LogFactory.getLog(GradleRepository.class);
    ...
    public GradleRepository(ThreadSource threadSource, CentralRepositoryDatabaseSerializer serializer) {
        this.pattern = new ExtraResultAnalysis();
        this.extra = new PatternResultAnalysis();
        this.terminate = false;
        this.database = new CentralRepositoryDatabase(threadSource, serializer);
    }
    ...
    public MavenSearchResult query(String pattern, int start) {
        this.terminate = false;
        String url = "https://plugins.gradle.org/search?term=" + StringUtils.trimBlank(StringUtils.replaceAll(pattern, ".", "%2E")) + "&page=" + (start - 1); // 构建请求 URL
