 ...
 */
public class IdeaMavenPluginListener implements MavenGeneralSettings.Listener, Disposable {

    private static volatile IdeaMavenPluginListener INSTANCE;
    ...
    public static void start(AnActionEvent event) {
        if (INSTANCE == null) {
            synchronized (IdeaMavenPluginListener.class) {
                if (INSTANCE == null) {
                    INSTANCE = new IdeaMavenPluginListener(event);
                }
            }
        }
    }
    ...
    protected IdeaMavenPluginListener(AnActionEvent event) {
    ...
    protected MavenProjectsManager execute(AnActionEvent event) {
        ...
        if (manager != null) {
            ...
            if (StringUtils.isNotBlank(filepath)) {
                LocalRepositoryConfig.REPOSITORY = new File(filepath); // 获取 Maven 本地仓库路径
            }
